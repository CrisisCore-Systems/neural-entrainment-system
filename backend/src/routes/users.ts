  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const result = await fastify.pg.query('SELECT * FROM user_preferences WHERE user_id = $1', [
          decoded.userId,
        ]);
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        await fastify.pg.query(`UPDATE user_preferences SET ${setClause} WHERE user_id = $1`, [
          decoded.userId,
          ...values,
        ]);
